...
- 子命令:
  ...
  - 辅助:`sit version`、`sit node enroll --token <t> --endpoint <url>`(首次接入引导)、`sit node setup`(Linux/macOS 自动接入)。
...
### macOS — launchd
`sit node setup` 在 macOS 上安装系统级 LaunchDaemon:

```
/Library/LaunchDaemons/com.meating.sit.plist
Label=com.meating.sit
ProgramArguments=/usr/local/bin/sit node --config /usr/local/etc/sit/node.yaml
UserName=_sit
GroupName=_sit
RunAtLoad=true
KeepAlive=true
ThrottleInterval=5
StandardOutPath=/usr/local/var/log/sit/node.log
StandardErrorPath=/usr/local/var/log/sit/node.err.log
```

默认创建 `_sit` 专用用户与 `_sit` group,`NFSHomeDirectory=/var/empty`,`UserShell=/usr/bin/false`;状态目录 `/usr/local/var/lib/sit` 与日志目录 `/usr/local/var/log/sit` 归 `_sit:_sit`。plist 必须归 `root:wheel` 且权限 `0644`,通过 `launchctl bootstrap system /Library/LaunchDaemons/com.meating.sit.plist` 加载。
...
- **自启样例**:`deploy/systemd/sit-node.service`、`deploy/launchd/com.meating.sit.plist`。
