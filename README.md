...
普通用户默认安装到 `~/.local/bin/sit`;root 用户默认安装到 `/usr/local/bin/sit`。可通过 `SIT_INSTALL_DIR=/path/to/bin` 覆盖安装目录。安装脚本会按本机平台下载最新 GitHub Release 中的预编译二进制,覆盖已有 `sit`,并打印安装后的版本。若安装目录不在 `PATH` 中,脚本会打印 shell 配置提示。
...
如果 REST API 与 Node WSS 不是同一个外部前缀,显式指定:

```bash
sudo sit node setup \
  --endpoint https://mgr.example/sit \
  --wss-endpoint wss://mgr.example/sit/connect \
  --token <enroll_token>
```

### 手动接入 Node(两阶段 enroll)
...
`web/` 提供一套纯静态、零构建的管理 GUI(原生 HTML+JS+极简 CSS),独立交付、不由 Manager 托管:登录、节点列表/详情、任务下发与结果、节点接入。实时刷新用轮询(非 WSS)。本地运行可用 `scripts/serve-web.sh` 启动 `http://localhost:45101/login.html`;跨域建议在 Manager API 前面的 Nginx 放行该 localhost origin,或用反代收敛同源。详见 [`web/README.md`](web/README.md)。
